...
export function OnboardingProfileForm({ user, onSaved }) {
  ...
  async function handleSubmit(event) {
    ...
    if (!form.consent) {
      setError("Please accept the consent and screening disclaimer to continue.");
      return;
    }
  ...
  return (
    <form className="onboarding-form" onSubmit={handleSubmit}>
            ...
            {diabetesSymptomOptions.map((item) => (
              <CheckboxChip
                ...
                onChange={() =>
                  setForm((current) => ({
                    ...current,
                    diabetesSymptoms: toggleValue(current.diabetesSymptoms, item),
                  }))
                }
              />
            ))}
      ...
      <label className="consent-card">
        ...
        />
        <span>
          I agree that this app is for screening and education only, not a diagnosis,
          and I consent to my data being stored securely for this care flow.
        </span>
      </label>
      ...
      <div className="dashboard-actions onboarding-actions">
        <button className="primary-button auth-button" disabled={pending} type="submit">
          {pending ? "Saving..." : "Save profile"}
        </button>
        <button className="ghost-button auth-switch" type="button" onClick={onSaved}>
          Skip for now
        </button>
      </div>
    </form>
  ...
  return (
    <form className="onboarding-form" onSubmit={handleSubmit}>
      ...
      <FieldGroup
        title="03"
        description="Medical history and routine"
      >
        <div className="field-grid two-col">
          <TextField
            label="Diet pattern"
